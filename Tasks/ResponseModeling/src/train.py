...
def preprocess_function(examples, max_target_length=64):

    queries = examples['query']
    history = examples['history_post']
    reponses = examples['response']
    # [CLS] [SEP]
    queries_inputs = tokenizer(queries, max_length=max_target_length, padding=False, truncation=True)
    reponses_inputs = tokenizer(reponses, max_length=max_target_length, padding=False, truncation=True)
    history_inputs = tokenizer(history, max_length=512, padding=False, truncation=True)

    ids1, mask1, token_type_ids1 = queries_inputs['input_ids'], queries_inputs['attention_mask'], queries_inputs['token_type_ids']
    ids2, mask2, token_type_ids2 = reponses_inputs['input_ids'], reponses_inputs['attention_mask'], reponses_inputs['token_type_ids']
    ids3, mask3, token_type_ids3 = history_inputs['input_ids'], history_inputs['attention_mask'], history_inputs['token_type_ids']
...
def validation(model, test_dataloader):
    ...
    with torch.no_grad():
        for _, data in enumerate(tqdm(test_dataloader, desc='process evaluate data')):
...
if __name__ == "__main__":  
    ...
    parser.add_argument("--history_post_path", default="/mnt/user/gaojingsheng/project/chitchat/chitchat/history_post_512.json", type=str)
    parser.add_argument("--load_model_path", default="", type=str)
    parser.add_argument("--output_dir", default="./outputs", type=str)
    parser.add_argument("--writer_dir", default="./runs", type=str)
    ...
    parser.add_argument("--train_id_num", default=150, type=int)
    parser.add_argument("--max_train_samples", default=4000, type=int)
    parser.add_argument("--max_val_samples", default=100, type=int)
    ...
    # args.do_train = True
    # args.do_eval = True
    # args.history_post = True
    args.add_id = True
    args.apply_interaction=True

    if args.do_train:
        writer = SummaryWriter(log_dir=args.writer_dir, flush_secs=120)

    if args.do_train:
        if args.train_id_num == 150:
            data_files["train"] = os.path.join(args.data_dir, "train_data.pk")
        elif args.train_id_num == 50:
            data_files["train"] = os.path.join(args.data_dir, "train50_data.pk")
        elif args.train_id_num == 15:
            data_files["train"] = os.path.join(args.data_dir, "train15_data.pk")
        else:
            raise KeyError

    if args.do_eval:
        if args.train_id_num == 15:
            data_files["validation"] = os.path.join(args.data_dir, "dev15_data.pk")
        else:
    ...
    for key in data_files:
        if args.history_post and args.add_id:
            ...
            persona_id_path = "./long_deprefix/personaId_list.json"
            datasets[key] = load_retrive_history_post_and_id(data_files[key], args.history_post_path, persona_id_path)

        elif args.history_post and not args.add_id:
            print("load history_post")
            datasets[key] = load_retrive_history_post(data_files[key], args.history_post_path)
    ...
    if args.do_train:
        ...
        print("Dataset length is {}".format(len(training_set)))
    ...
    if args.do_train:
        ...
        for epoch in range(args.epoch):
            model.train()
            for _, data in enumerate(tqdm(training_loader, desc='Processing')):
