...
def process_index_to_str(index, name):
    if name == "age":
        if index == 1:
            return 16
        elif index == 2:
...
def load_pk_persona(file_path, personaId_list_path):
    ...
    for sample in data:
        id_index = sample[0]
        query = str(id_list[id_index]) + " [SEP] "+ sample[1]
        response = sample[2]
        results['query'].append(query)
        results['response'].append(response)
    results = Ddataset.from_dict(results)
...
def load_pk_history_post(file_path, history_post_path):
    data = pk.load(open(file_path, 'rb'))
    results = {'history_post':[], 'query':[], 'response':[]}
    for sample in data:
        scratch_id = sample[0]
        query = sample[1]
        response = sample[2]
        results['history_post'].append(int(scratch_id))
        results['query'].append(query)
        results['response'].append(response)
    results = Ddataset.from_dict(results)
    return results
...
def load_retrive_history_post_and_id(file_path, history_post_path, persona_id_path):
    ...
    for sample in data:
        scratch_id = sample[0]
        query = str(id_list[scratch_id]) + " [SEP] "+ sample[1]
        response = sample[2]
        results['history_post'].append(history_post_list[scratch_id])
        results['query'].append(query)
        results['response'].append(response)
