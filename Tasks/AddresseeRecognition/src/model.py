...
class SingleBert(BertPreTrainedModel):
    def __init__(self, config):
        super(SingleBert, self).__init__(config)
        self.bert_model_config = config
        self.bert_tokenizer = BertTokenizer.from_pretrained("bert-base-chinese")  
        self.bert_model = BertModel.from_pretrained("bert-base-chinese", config=self.bert_model_config)
...
class ThreeBert(BertPreTrainedModel):
    def __init__(self, config, train_from_scratch=False):
        ...
        self.bert_model_config = config # BertConfig.from_pretrained("bert-base-chinese")
        self.bert_tokenizer = BertTokenizer.from_pretrained("bert-base-chinese")  
    ...
    def forward(self, ids, mask, token_type_ids, apply_interaction=True, aggregation_method="max"):
        ...
        # Comments: batch * candiate * length, batch_size = candidate
        batch_size, candiate_length, max_len = ids[1].size()

        # assert batch_size == candiate_length
        input_ids2 = ids[1].reshape(batch_size*candiate_length, max_len)
        attention_mask2 = mask[1].reshape(batch_size*candiate_length, max_len)
        token_type_ids2 = token_type_ids[1].reshape(batch_size*candiate_length, max_len)
        ...
        if len(ids)==3:
            ...
            if apply_interaction:
                ...
                batch_size, sent_len, emb_size = outputs1[0].shape
                history_output = outputs[0].repeat_interleave(candiate_length, dim=0)
                attention_mask3 = attention_mask3.repeat_interleave(candiate_length, dim=0)

                context_output = outputs1[0].repeat_interleave(candiate_length, dim=0)
                attention_mask1 = attention_mask1.repeat_interleave(candiate_length, dim=0)

                response_output = outputs2[0]
                attention_mask2 = attention_mask2

                logits = self.fuse(self.bert_model2, aggregation_method, \
                    context_output, response_output, history_output, attention_mask1, attention_mask2, attention_mask3, batch_size, candiate_length)
                targets = torch.tensor([candiate_length-1 for i in range(batch_size)], dtype=torch.long, device=self.device)
